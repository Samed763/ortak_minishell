...
/*   Updated: 2025/08/10 15:30:59 by sadinc           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
typedef struct s_command
{
	char					**args;
	pid_t					pid;
	t_redir *redirs;     // Bütün yönlendirmeler artık burada,
		sırayla tutulacak.
	t_heredoc *heredocs;
		// Heredoc içeriklerini tutmak için bu yapıyı koruyabiliriz.
...
void						write_error_and_exit(int exit_val, char *arg,
								char *error);
...
int							add_heredoc_to_command(t_command *cmd,
								char *raw_word);
void						add_redir_to_list(t_command *cmd, char *filename,
								int type);
